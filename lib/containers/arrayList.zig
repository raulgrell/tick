...
test "ArrayList" {
    ...
    list.reserve(8) catch unreachable;

    list.append(1) catch unreachable;
    list.prepend(0) catch unreachable;
    list.insert(2,2) catch unreachable;
    ...
    list.splice(1, 1, arr[0..]) catch unreachable;
    
    assert(list.data[1] == 6);
    assert(list.data[2] == 6);
    assert(list.data[3] == 6);

    assert(list.indexOf(eql, 0) == 0) catch unreachable;
