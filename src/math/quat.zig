pub fn QuatT(comptime T: type) -> type {
    struct {
        ...
        pub fn fromEulerAngles(angles: &Self) -> Self {
            const pitch = quat(1.0, 0.0, 0.0, angles.x);
            const yaw   = quat(0.0, 1.0, 0.0, angles.y);
            const roll  = quat(0.0, 0.0, 1.0, angles.z);
        ...
        pub fn sum(self: &const Self, other: &const Self) {
            quat(self.x + other.x, self.y + other.y, self.z + other.z, self.w + other.w);
        }
        pub fn diff(self: &const Self, other: &const Self) {
            quat(self.x - other.x, self.y - other.y, self.z - other.z, self.w - other.w);
        }
        pub fn prod(self: &const Self, other: &const Self) {
            normalize(quat(
        ...
        pub fn scaleUp(self: &const Self, scalar: T) {
            quat(self.x * scalar, self.y * scalar, self.z * scalar, self.w * scalar);
        }
        pub fn scaleDown(self: &const Self, scalar: T) {
            quat(self.x / scalar, self.y / scalar, self.z / scalar, self.w / scalar);
        }
        pub fn neg(self: &const Self, scalar: T) {
            quat(-self.x, -self.y, -self.z, -self.w);
        }

        pub fn rotationX(self: &Self, radians: T) {
            const angle = radians * 0.5;
            return quat(sin(angle), 0.0, 0.0, cos(angle));
        }
        pub fn rotationY(self: &Self, radians: T) {
            const angle = radians * 0.5;
            return quat(0.0, sin(angle), 0.0, cos(angle));
        }
        pub fn rotationZ(self: &Self, radians: T) {
            const angle = radians * 0.5;
            return quat(0.0, 0.0, sin(angle), cos(angle));
        }

        pub fn set(self: &Self, other: &Self) -> &Self {
            self.x = other.x;
            self.y = other.y;
            self.z = other.z;
            self.w = other.w;
        ...
        pub fn setXYZ(self: &Self, vec: &Vec3T(T)) -> &Self {
            self.x = vec.x;
            self.y = vec.y;
            self.z = vec.z;
            return self;
        ...
        pub fn getXYZ(self: &Self) -> Vec3T(T) {
            return Vec3T(T).init(self.x, self.y, self.z);
        }

        pub fn getElement(self: &Self, index: usize) -> T {
            return self.data[index];
        }

        pub fn getAxis(self: &Self) -> Vec3T(T) {
        ...
        pub fn toEulerAngles(self: &Self) -> Vec3T(T) {
        ...
        pub fn normalize(self: &Self) -> quat {
            const lenSqr = norm(self);
            const lenInv = rsqrt(lenSqr);
            return self * lenInv;
        }

        pub fn rotationVector(unitVec0: &const Vec3, unitVec1: &const Vec3) -> quat {
            const cosHalfAngleX2 = sqrt((2.0 * (1.0 + unitVec0.dot(unitVec1))));
            const recipCosHalfAngleX2 = (1.0 / cosHalfAngleX2);
            return quat((unitVec0.cross(unitVec1) * recipCosHalfAngleX2), (cosHalfAngleX2 * 0.5));
        }
        ...
        pub fn rotate(self: &Self, quat: &Self, vec: &Self) -> Vec3 {
        ...
        pub fn select(first: &Self, second: &Self, select_second: bool) -> quat {
            return quat(
