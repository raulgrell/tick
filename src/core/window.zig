...
extern fn window_close_callback(win: ?&c.GLFWwindow) {
    c.glfwSetWindowShouldClose(win, c.GLFW_TRUE);
}

extern fn window_size_callback(win: ?&c.GLFWwindow, width: c_int, height: c_int) {
    // User or system resized window
}

extern fn file_drop_callback(win: ?&c.GLFWwindow, count: c_int, paths: ?&?&const u8) {
