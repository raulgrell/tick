...
// const GUIManager = struct  {

//     Panel mScreenContainer;
//     GUIElement* mFocusedElement;
//     bool mMouseInteraction;
//     Camera2D mCamera;
//     Renderer2D mRenderer;


//     void init() {
//         mCamera.create(Rectangle());
//         mRenderer.init(false);
//         mRenderer.setCamera(&mCamera);
//     }

//     void dispose() {
//         mScreenContainer.dispose();
//         mRenderer.dispose();
//     }
...
//         mScreenContainer.calculatePosAndSize();
//         mScreenContainer.update(dt, inputManager);
//     }

//     void render() {
//         mRenderer.open();

//         mRenderer.submit(&mScreenContainer);
...
//         mScreenContainer.addElement(element);
//     }

//     void focusOn(GUIElement * element) {
//         mFocusedElement->setFocus(false);
//         element->setFocus(true);
//         mFocusedElement = element;
//     }

//     void removeFocus(GUIElement * element) {
...
//     bool isInFocus(GUIElement * element) {
//         return mFocusedElement == element;
//     }
...
//     bool willMouseInteract() const {
//         return mMouseInteraction;
//     }

//     void setMouseInteraction(bool value) {
//         mMouseInteraction = value;
//     }

//     void updateWindowSize(float width, float height) {
//         mScreenContainer.setSize(Vector2(width, height));
//         mCamera.resize(Rect(0.0, 0.0, width, height));
//     }

// }

// const GUIContainer = struct : public GUIElement {
//     protected:
//         vector<GUIElement*> mChildElements; // vector of child elements. Data is assumed to be on the heap and in managed by this container
//         //layout?
//     };

//     void dispose() {
//         for(size_t i = 0; i < mChildElements.size(); i++) {
//             delete mChildElements[i];
//         }
...
//         if(mParent) {
//             mPos = (mRelativePos * mParent->getSize()) + mParent->getPos();
//             mSize = mRelativeSize * mParent->getSize();
//         }

//         //apply aspect
//         if(mUseAspectWidth) {
//             mSize.h = mAspectRatio / mSize.w;
//         }
//         else if(mUseAspectHeight) {
//             mSize.w = mAspectRatio * mSize.h;
//         }
...
//         for(size_t i = 0; i < mChildElements.size(); i++) {
//             mChildElements[i]->calculatePosAndSize();
//         }
//     }
// }

// const GUIElement = struct : public IDrawable2D {
//     protected:
//         Vector2 mPos;  // absolute pixel coords in screen-space
//         Vector2 mSize; // absolute pixel size

//         Vector2 mRelativePos;  //normalized pos of the element relative to the parent
//         Vector2 mRelativeSize; //normalized size of the element relative to the parent

//         GUIElement* mParent = nullptr; // reference to this element's parent element

//         GUIManager* mGUIManager = nullptr; // reference to this element's GUIManager
...
//         bool mCenterX;
//         bool mCenterY;
//         bool mUseAspectWidth;
//         bool mUseAspectHeight;
//         float mAspectRatio;
...
//         mCenterX = false;
//         mCenterY = false;
//         mUseAspectWidth = false;
//         mUseAspectHeight = false;
//         mAspectRatio = 1.0;
//     }
...
//     void setRelativePos(Vector2 relPos) {
//         mRelativePos = relPos;
//         mUsePixelPos = false;
//     }

//     void setRelativeSize(Vector2 relSize) {
//         mRelativeSize = relSize;
//         mUsePixelSize = false;
//     }
...
//     void centerXPos(bool center) {
//         mCenterX = center;
//     }

//     void centerYPos(bool center) {
//         mCenterY = center;
//     }

//     void useAspectWidth(bool use) {
//         mUseAspectWidth = use;
//     }

//     void useAspectHeight(bool use) {
//         mUseAspectHeight = use;
//     }
...
//         if(!mUsePixelPos)  mPos = (mRelativePos * mParent->getSize()) + mParent->getPos();
//         if(!mUsePixelSize) mSize = mRelativeSize * mParent->getSize();
...
// const Panel = struct : public GUIContainer {

//     void update(float dt, InputManager & inputManager) {

//         for(size_t i = 0; i < mChildElements.size(); i++) {
//             if(mChildElements[i]->isVisible()) {
//                 mChildElements[i]->update(dt, inputManager);
//             }
//         }
//     }

//     void render(IRenderer2D * renderer) const {
    
//         for(size_t i = 0; i < mChildElements.size(); i++) {
//             if(mChildElements[i]->isVisible()) {
//                 mChildElements[i]->render(renderer);
//             }
//         }
//     }
// }

// const Button = struct : public GUIElement {

//         Sprite mTextures;
//         mDefaultTint: ColorRGBA8;
//         mHoverTint: ColorRGBA8;
//         mPressedTint: ColorRGBA8;

//         Vector2 mHoverScale;
//         Vector2 mPressedScale;

//         Label mLabel;
//         mDefaultTextColor: ColorRGBA8;
//         mHoverTextColor: ColorRGBA8;
//         mPressedTextColor: ColorRGBA8;
...
//     Button() {
//         mDefaultTint = VOLT_COLOR_WHITE;
//         mHoverTint   = VOLT_COLOR_WHITE;
//         mPressedTint = VOLT_COLOR_WHITE;
//         mPressed = false;

//         mHoverScale   = Vector2(1.2, 1.0);
//         mPressedScale = Vector2(1.2, 1.0);
//     }
...
//     void setTextures(filePath: string, defaultRect: &const Rectangle, hoverRect: &const Rectangle, pressedRect: &const Rectangle, inActiveRect: &const Rectangle) {
//         mTextures.bindPos(&mPos);
//         mTextures.loadTexture(filePath);

//         mTextures.resetVerticies(mSize);

//         mTextures.addFrame(defaultRect, Vector2(0.0), 0.0);
//         mTextures.addFrame(hoverRect, Vector2(5.0), 0.0);
//         mTextures.addFrame(pressedRect, Vector2(10.0), 0.0);
//         mTextures.addFrame(inActiveRect, Vector2(10.0), 0.0);
//         mTextures.stop();
//     }

//     void setText(str: string, fontPath: string, uint size, defColor: ColorRGBA8, hoverColor: ColorRGBA8, pressedColor: ColorRGBA8) {
//         mDefaultTextColor = defColor;
//         mHoverTextColor = hoverColor;
//         mPressedTextColor = pressedColor;
//         mLabel.create(str, fontPath, size, defColor);
//         mLabel.setParent(this);
        
//     }

//     void setTextOffset(const Vector2& offset, bool centerX, bool centerY) {
//         mLabel.setRelativePos(offset);
//         mLabel.centerXPos(centerX);
//         mLabel.centerYPos(centerY);
//     }
...
//         mPressed = false;
//         mLabel.calculatePosAndSize();
//         mLabel.update(dt, inputManager);
//         mTextures.resetVerticies(mSize);
//         mTextures.setDepth(0.2);
//         mTextures.bindPos(&mPos);
//         mTextures.setOrigin(Vector2(mSize.x/2.0, mSize.y/2.0));
//         Vector2 mp = inputManager.getMousePos();

//         if(!mActive) {
//             mTextures.setFrame(3); // set to deactive texture
//             mTextures.setTint(mDefaultTint);
//             mTextures.setScale(Vector2(1.0));
//             mLabel.getText()->setTint(mDefaultTextColor);
//             return;
//         }

//         if(mp.x > mPos.x && mp.x < mPos.x + mSize.w && mp.y > mPos.y && mp.y < mPos.y + mSize.h) { // mouse over button
//             mTextures.setFrame(1); // set to hover texture
//             mTextures.setTint(mHoverTint);
//             mTextures.setScale(mHoverScale);
//             mLabel.getText()->setTint(mHoverTextColor);

//             if(inputManager.isButtonDown(MouseLeft)) {
//                 mTextures.setFrame(2); // set to pressed texture
//                 mTextures.setTint(mPressedTint);
//                 mTextures.setScale(mPressedScale);
//                 mLabel.getText()->setTint(mPressedTextColor);
//             }
//             else if(inputManager.wasButtonReleased(MouseLeft)) {
//                 mTextures.setFrame(1); // set to hover texture
//                 mTextures.setTint(mHoverTint);
//                 mTextures.setScale(mHoverScale);
//                 mLabel.getText()->setTint(mHoverTextColor);
...
//         }
//         else {
//             mTextures.setFrame(0); // set to default texture
//             mTextures.setTint(mDefaultTint);
//             mTextures.setScale(Vector2(1.0));
//             mLabel.getText()->setTint(mDefaultTextColor);
//         }
